...
class image_converter:
    ...
    def detect_target_pos(self,image,cam1=True):
        ...
        sphere_position = self.detect_target(image,True)
        box_position = self.detect_target(image,False)
        ...
        if sphere_position is None:
            ...
            sphere_position = np.array([self.target1_history[index1], self.target1_history[index2]])
        else:
            self.target1_history[index1] = sphere_position[0]
            self.target1_history[index2] = sphere_position[1]

        if box_position is None:
            # when the target can not be viewd clearly, use previous position of the target
            box_position = np.array([self.target2_history[index1], self.target2_history[index2]])
        else:
            self.target2_history[index1] = box_position[0]
            self.target2_history[index2] = box_position[1]
        ...
        sphere_position = base - a * sphere_position
        box_position = base - a * box_position
        # get the position of end effector for the control problem
        end_effector_pos = base - a * self.detect_red(image)

        return sphere_position, box_position, end_effector_pos
    ...
    def image1_callback(self, data):
        ...
        sphere_position, box_position , end_effector_pos= self.detect_target_pos(self.cv_image1,cam1=True)

        # publish the y coordinates and z coordiantes of the sphere target and square target
        self.target1_y_pos.data = -sphere_position[0]
        self.target1_z_pos.data = sphere_position[1]
        self.target2_y_pos.data = -box_position[0]
        self.target2_z_pos.data = box_position[1]
    ...
    def image2_callback(self, data):
        ...
        sphere_position, box_position, end_effector_pos = self.detect_target_pos(self.cv_image2,cam1=False)

        # publish the x coordinates of the sphere target and square target
        self.target1_x_pos.data = -sphere_position[0]
        self.target2_x_pos.data = -box_position[0]
